...
class HashingProblems {
    ...
    public double getAverage(HashMap<Integer, Integer> map, int[] array) {
        ...
        if (count == 0) {
            return 0.0 / 0.0;  // Returns NaN if no common keys are found
        }
    ...
    public int twoSums(int[] numbers, int k) {
        ...
        for (int number : numbers) {
            // Check if (number + k) or (number - k) exists in the set
            if (seenNumbers.contains(number + k)) {
                count++;  // (number + k) forms a valid pair
            }
            if (seenNumbers.contains(number - k)) {
                count++;  // (number - k) forms a valid pair
            }
