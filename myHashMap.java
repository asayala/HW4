...
class myHashMap<K,V> {
    ...
    public V replace(K key, V val) {
        ...
        // Traverse the bucket's linked list to find the key
        while (head != null) {
            if (head.key.equals(key)) {
                // Found the key, replace the value
                V oldValue = head.value;
                head.value = val;  // Use val instead of newVal
                return oldValue;
            }
            head = head.next;
        }
    ...
    public boolean replace(K key, V oldVal, V newVal) {
        ...
        while (head != null) {
            if (head.key.equals(key) && head.value.equals(oldVal)) {
                // Found the key with the expected old value, replace the value
                head.value = newVal;
                return true;
            }
            head = head.next;
        }
        return false; // Key not found or value does not match
    }
